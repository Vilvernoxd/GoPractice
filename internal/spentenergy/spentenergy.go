...
func WalkingSpentCalories(steps int, weight, height float64, duration time.Duration) (float64, error) {
	...
	calories := ((walkingCaloriesWeightMultiplier * weight) + (avgSpeed*avgSpeed/height)*walkingSpeedHeightMultiplier) * durationInHours * minInH
...
func RunningSpentCalories(steps int, weight float64, duration time.Duration) (float64, error) {
	...
	calories := ((runningCaloriesMeanSpeedMultiplier * avgSpeed) - runningCaloriesMeanSpeedShift) * weight
...
func Distance(steps int) float64 {
	...
	distInKm := (float64(steps) * lenStep) / mInKm
	return distInKm
}
